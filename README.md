...
- 측정 시 원시 마이크 신호를 보관 → **WAV 저장**(16-bit PCM) 다운로드
- **전체 재생** / **정지**, **대역 재생**(중심주파수 Hz + Q, bandpass 필터로 특정 주파수만 청취)
- 프리셋: CV1 / CV2 / Cavity중심 (중심주파수 자동 입력)
- 음원은 **IndexedDB**에 측정별 보관 → 저장결과를 선택하면 그 측정의 음원도 재생/저장 가능
- 재생은 원시신호가 작아 자동 정규화(피크 기준)해 들리게 함 (저장 WAV는 원본 유지)
